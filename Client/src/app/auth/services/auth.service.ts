...
export class AuthService {
  ...
  logOut() {
    ...
    const clientId = this.getClientId();
    //Cho User thành null để thành trạng thái đăng xuất nhưng không xóa token vì xóa đi server biết thằng nào mà log
    this.updateUser(null);
    //Chuyển hướng trang về trang hiện tại sau khi logout
    this.router.navigateByUrl(currentUrl).then(() => {
      ...
      this.http.post(this.urlAuth + '/logout', {userId, clientId}).subscribe({
        ...
        error: (err) => {
          ...
          this.updateUser(null);
          //this.reloadWindow();
          console.log("Lỗi phản hồi từ server khi logout", err);
        }
      });
    })
  }
